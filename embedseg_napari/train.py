...
from embedseg_napari.utils.utils import AverageMeter, Cluster, Cluster_3d, Logger, Visualizer, prepare_embedding_for_train_image
...
def train_vanilla(model, train_dataset_it, criterion, optimizer, cluster, display, display_embedding, display_it, one_hot, grid_x, grid_y, pixel_x, pixel_y, n_sigma, color_map,
                  args, progress_bar = None, viewer=None, fig=None):  # this is without virtual batches!
    ...
    for i, sample in enumerate(tqdm(train_dataset_it)):
        ...
        if progress_bar is not None:
            progress_bar.setValue(np.round(100 * (i + 1) / len(train_dataset_it)).astype(int))
        ...
        #         yield predictions, instance, predictions.cpu()
...
def train_vanilla_3d(model, train_dataset_it, criterion, optimizer, cluster, display, display_embedding, display_it, one_hot, grid_x, grid_y, grid_z, pixel_x, pixel_y, pixel_z, n_sigma,
                     zslice, args, progress_bar = None, viewer=None, fig=None):  # this is without virtual batches!
...
def val_vanilla(model, val_dataset_it, criterion, optimizer, cluster, display, display_embedding, display_it, one_hot, grid_x, grid_y, pixel_x, pixel_y, n_sigma, args, progress_bar=None):
    # define meters
    loss_meter, iou_meter = AverageMeter(), AverageMeter()
    # put model into eval mode
    model.eval()
    with torch.no_grad():
        for i, sample in enumerate(tqdm(val_dataset_it)):
            ...
            loss = loss.mean()
            if progress_bar is not None:
                progress_bar.setValue(np.round(100 * (i + 1) / len(val_dataset_it)).astype(int))
            # if display and i % display_it == 0:
            #     with torch.no_grad():
...
def val_vanilla_3d(model, val_dataset_it, criterion, optimizer, cluster,  display, display_embedding, display_it, one_hot, grid_x, grid_y, grid_z, pixel_x, pixel_y, pixel_z, n_sigma, zslice, args,
                   progress_bar = None):
    # define meters
    loss_meter, iou_meter = AverageMeter(), AverageMeter()
    # put model into eval mode
...
def create_random(i):
    return np.random.rand(512, 512) * np.cos(i * 0.2)

def begin_training(train_dataset_dict, val_dataset_dict, model_dict, loss_dict, configs, color_map='magma', progress_bars=None, viewer=None):
    ...
    for epoch in range(start_epoch, configs['n_epochs']):
        ...
        if(len(progress_bars)==3):
            if (configs['grid_z'] is None):
                if (train_dataset_dict['virtual_batch_multiplier'] > 1):
                    ...
                    train_loss = train_vanilla(display=configs['display'], display_embedding=configs['display_embedding'],
                                               ...
                                               pixel_x=configs['pixel_x'], pixel_y=configs['pixel_y'], color_map=color_map,
                                               args=loss_dict['lossW'], progress_bar=progress_bars[1], viewer= viewer, fig=fig)
                ...
                if (val_dataset_dict['virtual_batch_multiplier'] > 1):
                    ...
                    val_loss, val_iou = val_vanilla(display=configs['display'], display_embedding=configs['display_embedding'],
                                                    ...
                                                    args=loss_dict['lossW'], progress_bar = progress_bars[2])
                progress_bars[1].setValue(0)
                progress_bars[2].setValue(0)
            ...
            progress_bars[0].setValue(np.round(100 * (epoch + 1) / configs['n_epochs']).astype(int))
